...
## 6. Safety / trust features already built
- Block user (bidirectional — blocked user can't message/see blocker)
- Report user, report event, report individual chat message
- Message unsend (any time) and edit (within 15 minutes, edited-at timestamp shown)
- Delete-for-me (per-message, local-only deletion)
- Discoverability opt-out ("Show me in Discover" toggle)
- Chat requires mutual accepted Vybe request — no cold-DM messaging
- Account deletion requires OTP re-verification (can't be triggered by a stolen session alone)

---

## 7. Third-party open-source libraries (client + server)

Full credits list lives in `(settings)/open-source.tsx`. Notable ones:
React Native, React, Expo SDK (+ expo-audio/video/camera/location/
notifications/image/secure-store/router/etc.), react-native-reanimated,
react-native-gesture-handler, react-native-webview, react-native-maps,
MapLibre React Native, @gorhom/bottom-sheet, @shopify/flash-list,
lucide-react-native, zustand — client side. FastAPI, PostgreSQL, Redis,
Razorpay SDK, Twilio SDK, Pillow, OpenCV — server side. Full dependency
lists are in `client/package.json` and `server/requirements.txt`.

---

## 8. Account deletion — how it actually works now

- User taps Delete Account → OTP re-verification → soft-delete: `is_deleted =
  TRUE`, `deleted_at = NOW()`, name/bio/voice_url blanked, RSVPs cancelled,
  and every refresh token for that user is revoked server-side (real
  logout, not just the client clearing local state) — `DELETE /users/me` in
  `server/routes/users.py`.
- Blocked entirely (HTTP 409) if the user has upcoming hosted events —
  they must cancel those first.
- **Restore within 30 days**: manual, support-driven. User emails
  support@vybe.in, support flips `is_deleted` back to `FALSE` in the DB.
  No self-service restore endpoint — by design.
- **After 30 days**: a background job (`utils/account_purge.py`, run daily
  via a startup loop in `main.py`) hard-deletes the account permanently.
  It deletes the user's R2-stored files first (profile photos,
  voice-intro recording), then runs `DELETE FROM users WHERE id = ...`,
  which cascades via `ON DELETE CASCADE` through every dependent table
  (messages, conversations, follows, vibe requests, wallet transactions,
  payment orders, reviews, reports, device tokens, etc.). Past 30 days,
  restore is impossible — matches the "removed within 30 days" copy in
  `delete-account.tsx` exactly now.
- **Known side effect, accepted as intended**: `events.host_id` also
  cascades, so past events that user hosted are deleted too — including
  other attendees' RSVP history, reviews, and payment records for those
  specific events. This was a deliberate simplicity trade-off, not an
  oversight.
- **Not yet cleaned up**: event cover photos (`events.cover_photos`, a
  JSON array of full URLs rather than an R2 key) aren't deleted from R2
  storage when their event cascades away — a minor orphaned-file gap, low
  priority since it's just storage cost, not a privacy/legal issue (the
  DB rows themselves are gone).
