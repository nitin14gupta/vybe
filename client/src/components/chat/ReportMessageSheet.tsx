...
function ReportMessageSheetCore({ onSubmit, onClose }: Omit<Props, 'visible'>) {
    ...
    >
      <BottomSheetView style={s.content}>
        <View style={s.headerRow}>
          <View>
            <Text style={s.title}>Report message</Text>
            <Text style={s.subtitle}>We won't let them know you reported this.</Text>
          </View>
...
export function ReportMessageSheet({ visible, ...rest }: Props) {
  if (!visible) return null
  return <ReportMessageSheetCore {...rest} />
}
...
const s = StyleSheet.create({
  ...
  options: { borderRadius: 16, borderWidth: 1, borderColor: withOpacity(Colors.inkPrimary, 0.08), overflow: 'hidden', marginBottom: 20 },
  option: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', paddingHorizontal: 18, paddingVertical: 18 },
  optionBorder: { borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: withOpacity(Colors.inkPrimary, 0.08) },
  optionText: { fontFamily: FontFamily.bodyRegular, fontSize: 16, color: Colors.inkPrimary },
  radio: { width: 24, height: 24, borderRadius: 12, borderWidth: 2, borderColor: withOpacity(Colors.inkPrimary, 0.25), alignItems: 'center', justifyContent: 'center' },
