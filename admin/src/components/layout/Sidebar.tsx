      ...
      {NAV_GROUPS.map((group, groupIndex) => {
        ...
        return (
          <div key={group} className={groupIndex > 0 ? 'mt-3' : undefined}>
            ...
            {!collapsed && (
              <p className="mb-1.5 px-3 font-sans text-[11px] font-semibold uppercase tracking-wide text-ink-secondary">
                {group}
              </p>
            )}
...
export function Sidebar() {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'hidden shrink-0 flex-col border-r border-divider bg-background py-4 transition-all duration-200 md:flex',
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 flex h-full w-64 flex-col border-r border-divider bg-background py-4">
